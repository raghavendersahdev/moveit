...
#include "stomp_planning_adapter.h"
...
{
StompPlanningAdapter::StompPlanningAdapter() : planning_request_adapter::PlanningRequestAdapter(), nh_("~")
{
}

StompPlanningAdapter::~StompPlanningAdapter()
{
}

bool StompPlanningAdapter::initialize(const robot_model::RobotModelConstPtr& model, const std::string& ns)
{
  if (!ns.empty())
  {
...
bool StompPlanningAdapter::canServiceRequest(const moveit_msgs::MotionPlanRequest& req) const
{
...
void StompPlanningAdapter::getPlanningAlgorithms(std::vector<std::string>& algs) const
{
  algs.clear();
  if (!planners_.empty())
  {
...
planning_interface::PlanningContextPtr StompPlanningAdapter::getPlanningContext(
...
CLASS_LOADER_REGISTER_CLASS(stomp_moveit::StompPlanningAdapter, planning_request_adapter::PlanningRequestAdapter)
