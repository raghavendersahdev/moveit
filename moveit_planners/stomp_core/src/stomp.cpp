...
static void computeLinearInterpolation(const std::vector<double>& first, const std::vector<double>& last,
                                       int num_timesteps, Eigen::MatrixXd& trajectory_joints)
{
 ...
 */
static void computeCubicInterpolation(const std::vector<double>& first, const std::vector<double>& last, int num_points,
                                      double dt, Eigen::MatrixXd& trajectory_joints)
{
 ...
 */
bool computeMinCostTrajectory(const std::vector<double>& first, const std::vector<double>& last,
                              ...
                              const Eigen::MatrixXd& inv_control_cost_matrix_R, Eigen::MatrixXd& trajectory_joints)
{
  ...
  for (unsigned int d = 0; d < first.size(); d++)
  {
    ...
    linear_control_cost[d].transpose() += last[d] * Eigen::VectorXd::Ones(FINITE_DIFF_RULE_LENGTH - 1).transpose() *
                                          control_cost_matrix_R_padded.block(end_index_padded + 1, start_index_padded,
 ...
 */
void computeParametersControlCosts(const Eigen::MatrixXd& parameters, double dt, double control_cost_weight,
                                   const Eigen::MatrixXd& control_cost_matrix_R, Eigen::MatrixXd& control_costs)
{
...
{
Stomp::Stomp(const StompConfiguration& config, TaskPtr task) : config_(config), task_(task)
{
  resetVariables();
}
...
bool Stomp::solve(const Eigen::MatrixXd& initial_parameters, Eigen::MatrixXd& parameters_optimized)
{
  ...
  if (parameters_valid_)
  {
    ROS_INFO("STOMP found a valid solution with cost %f after %i iterations", current_lowest_cost_, current_iteration_);
  }
  else
  {
    if (proceed_)
...
bool Stomp::resetVariables()
{
  ...
  generateFiniteDifferenceMatrix(num_timesteps_padded_, DerivativeOrders::STOMP_ACCELERATION, config_.delta_t,
  ...
  control_cost_matrix_R_ = control_cost_matrix_R_padded_.block(start_index_padded_, start_index_padded_,
                                                               config_.num_timesteps, config_.num_timesteps);
...
bool Stomp::computeInitialTrajectory(const std::vector<double>& first, const std::vector<double>& last)
{
  ...
  switch (config_.initialization_method)
  {
    ...
    case TrajectoryInitializations::MININUM_CONTROL_COST:
      ...
      valid = computeMinCostTrajectory(first, last, control_cost_matrix_R_padded_, inv_control_cost_matrix_R_,
...
bool Stomp::generateNoisyRollouts()
{
  ...
  {
    if (!task_->generateNoisyParameters(parameters_optimized_, 0, config_.num_timesteps, current_iteration_, r,
                                        noisy_rollouts_[r].parameters_noise, noisy_rollouts_[r].noise))
    {
...
bool Stomp::filterNoisyRollouts()
{
  ...
  {
    if (!task_->filterNoisyParameters(0, config_.num_timesteps, current_iteration_, r,
...
bool Stomp::computeRolloutsStateCosts()
{
  ...
  for (auto r = 0u; r < config_.num_rollouts; r++)
  {
    ...
    Rollout& rollout = noisy_rollouts_[r];
    if (!task_->computeNoisyCosts(rollout.parameters_noise, 0, config_.num_timesteps, current_iteration_, r,
...
bool Stomp::computeRolloutsControlCosts()
{
  ...
  for (auto r = 0u; r < num_active_rollouts_; r++)
  {
    ...
    {
      computeParametersControlCosts(rollout.parameters_noise, config_.delta_t, config_.control_cost_weight,
...
bool Stomp::updateParameters()
{
  ...
  // filtering updates
  if (!task_->filterParameterUpdates(0, config_.num_timesteps, current_iteration_, parameters_optimized_,
                                     parameters_updates_))
  {
...
bool Stomp::computeOptimizedCost()
{
  ...
  if (config_.control_cost_weight > MIN_CONTROL_COST_WEIGHT)
  {
    computeParametersControlCosts(parameters_optimized_, config_.delta_t, config_.control_cost_weight,
  ...
  // state costs
  if (task_->computeCosts(parameters_optimized_, 0, config_.num_timesteps, current_iteration_, parameters_state_costs_,
                          parameters_valid_))
  {
